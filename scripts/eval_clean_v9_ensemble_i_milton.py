...
Reads:
  ...
  checkpoints_clean_v9_622_i_s{42,43,44}/best_model.pt

Writes:
  analysis_results/clean_v9_ensemble_eval_i_milton/results.json + summary.csv
"""
...
OUT_DIR   = os.path.join(PROJECT, "analysis_results/clean_v9_ensemble_eval_i_milton")
os.makedirs(OUT_DIR, exist_ok=True)

SEEDS     = [42, 43, 44]
K_EVAL    = 128            # standard I-block eval K (matches baseline eval)
N_SEEDS   = 15             # aligned with clean/ntrain/K-sweep/A+B evals
BATCH_QRY = 4  # v9 feat attn heavy — smaller batch             # per-protein attn deeper: shrink batch
N_EVAL_MAX = 10000
...
def load_v7(ckpt_path, device):
    ...
    mc = cfg.get("model", {})
    model = ICLTransformerV9FeatAttn(
...
def main():
    ...
    ckpts = [os.path.join(PROJECT, f"checkpoints_clean_v9_622_i_s{s}/best_model.pt")
             for s in SEEDS]
    ...
    clean_dir = os.path.join(PROJECT, f"processed_data_clean_i_milton_622_{letter_lower}")
    ...
    print(f"\n{'='*70}\n  Block I — v9 featattn ENSEMBLE, K={K_EVAL}, "
    ...
    for code in block_out["test"]:
        ...
        m = ensemble_eval(models, Xc, yc, Xq, yq, device, K_EVAL)
        results[key] = {
    ...
    for metric in ["auroc","auprc","brier","ece"]:
        v = df[metric].dropna()
        if len(v): print(f"  I-block mean {metric}: {v.mean():.4f}", flush=True)
