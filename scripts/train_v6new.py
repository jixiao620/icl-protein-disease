"""
Training script for ICL Transformer v6 (set-based, bidirectional, per-context norm).
Uses same training diseases as v5 for fair comparison.
Trains from scratch — no GPT-2 pretraining.
"""
...
def main():
    ...
    print(f"\nICLTransformerV6: {n_params:,} parameters", flush=True)
    print(f"  Bidirectional attention: yes", flush=True)
    print(f"  Context PE: none (set-based)", flush=True)
    print(f"  Label slots: 3 (neg/pos/unknown)", flush=True)
    print(f"  Per-context normalization: yes", flush=True)
